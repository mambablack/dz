...
assert.equal(
  bestHand([
    card('A', 's'), card('K', 's'), card('Q', 's'), card('J', 's'), card('T', 's'), card('2', 'd'), card('3', 'c'),
  ]).name,
  '同花顺',
);
assert.equal(
  bestHand([
    card('9', 's'), card('9', 'h'), card('9', 'd'), card('9', 'c'), card('A', 's'), card('K', 'd'), card('2', 'c'),
  ]).name,
  '四条',
);
assert.ok(preflopStrength([card('A', 's'), card('A', 'h')]) > preflopStrength([card('7', 's'), card('2', 'h')]));

let game = createReadyGame();
for (let hand = 0; hand < 120; hand += 1) {
  game = startNextHand(game);
  const cards = [...game.players.flatMap((player) => player.holeCards), ...game.deck];
  assert.equal(new Set(cards.map(cardText)).size, 52, '每手牌必须使用一副无重复的 52 张牌');
  const startingTotal = game.startingStacks.reduce((sum, stack) => sum + stack, 0);
  let guard = 0;
  while (game.status === 'playing' && guard < 250) {
    ...
    if (actor === 0) {
      ...
      if (hand % 17 === 0 && hero.streetBet + hero.stack > game.currentBet) {
        game = applyAction(game, 0, { type: 'raise', raiseTo: hero.streetBet + hero.stack });
      } else {
  ...
  assert.ok(game.players.every((player) => player.stack >= 0), '筹码不能为负数');
  assert.equal(game.players.reduce((sum, player) => sum + player.stack, 0), startingTotal, '单手结算后筹码总额必须守恒');
